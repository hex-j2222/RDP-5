 ...
 * alive (and show a persistent notification) while a remote session
 * (RDP, VNC, or SSH) is connected, so the connection is not torn down by the
 * OS when the user switches apps or locks the screen (issue #9 — "the
 * session should run in the background so the connection is not lost").
 *
 * The actual [com.gotohex.rdp.remote.RemoteSessionClient] continues to run
 * inside [RdpSessionViewModel] (via `viewModelScope`, on `Dispatchers.IO`),
 * which is unaffected by the activity's UI lifecycle as long as the process
 * stays alive — this service's purpose is exactly to keep that process alive.
 */
